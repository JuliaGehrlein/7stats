...
#best fit NSI parameter
epseu=    2.02366521e-01
epsmu=  -2.95015935e-02
  

#calculate the signal using the best fit NSI parameters
...
def randompred(sig,n,bkg):
 #if one wants to use the new pull term parametrization uncomment the next two lines
#        alpha=stat.pulldist(0.28)
 #       beta=stat.pulldist(0.25)
    ...
    #if smearing is required uncomment the next line
...
#apply Poisson fluctuations to the predicted number of events in each bin
...
#        smeared=acc.smearing_fct(sig_n)
       ...
       #if the new pull term parametrization is used uncomment the next line
